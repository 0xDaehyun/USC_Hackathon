...
export function LiveMap({
    ...
    const onLoad = () => {
      ...
      void (async () => {
        try {
          ...
          const seedPred = PREDICTION_STEPS.at(-1)?.polygon ?? null;
          setSourceData(
            map,
            "silos-buildings-3d",
            buildingsForExtrusion(
              buildings,
              buildings3dClip(FIRE_BOUNDARY, seedPred, FIRE_3D_BUFFER_M),
              {
                firePoly: FIRE_BOUNDARY,
                predPoly: seedPred,
                fireOn: true,
                predictionOn: true,
              },
  ...
  useEffect(() => {
    ...
    const firePoly = fire?.boundary_geojson ?? FIRE_BOUNDARY;
    const predPoly =
        ...
        {attribOpen && (
          <div className="s-panel absolute right-0 bottom-10 w-64 p-3 text-[11px] leading-relaxed text-[var(--s-type-2)]">
            ...
            <p>
              ...
              prediction polygons are simulated for the SILOS demo.
            </p>
          </div>
        )}
      </div>
