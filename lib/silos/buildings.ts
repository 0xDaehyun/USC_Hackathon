...
export function pointInPolygon(
  ...
  return polygons.some((coordinates) => {
    ...
    if (!exterior?.length || !ringContains(exterior, lng, lat)) {
      return false;
    }
    return coordinates
      .slice(1)
      .every((hole) => !ringContains(hole, lng, lat));
  });
}
...
export function buildingsForExtrusion(
  ...
  opts: {
    ...
    predPoly: Feature<Polygon | MultiPolygon> | null;
    fireOn: boolean;
...
export function buildings3dClip(
  ...
  if (predPoly) {
    if (predPoly.geometry.type === "MultiPolygon") {
      return predPoly;
    }
    return bufferPolygonMeters(
      predPoly.geometry,
      Math.min(250, bufferM * 0.25),
    );
  }
